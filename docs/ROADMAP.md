# 分阶段实施路线

## 已完成的基础

- 本机 `relocate-mug` 训练和策略可视化已经跑通。
- 复用 DexMV 仿真和 dexmv-learn，不复制大型资源。
- 已有环境检查、视频抽帧、retarget、可视化、demonstration 生成与检查脚本。
- 已有 DAPG 训练和策略回放入口。

## 阶段 1：获得一条真实杯子轨迹

状态：已完成。Subject 01 数据保存在共享盘，已选用 `20200709_150949` 的
`840412060917` 相机视角。

需要完成：

- 下载一个 DexYCB 受试者数据包、标定文件和 YCB 模型。
- 筛选右手 `025_mug` 轨迹。
- 选择一个无遮挡的固定相机视角。
- 记录原始序列名称、相机编号和数据许可。

完成标准：

- 能连续播放“手静止 -> 接近 -> 抓住 -> 举起杯子”的 RGB-D 序列。
- 能读取内参、外参、深度、手部标签和物体标签。

## 阶段 2：转换成世界坐标轨迹

状态：真实数据扫描、AprilTag 桌面坐标转换、重投影和 MANO
精确旋转对比均已完成。

需要完成：

- 编写 DexYCB 序列扫描和 `025_mug` 筛选程序。
- 转换 MANO、3D 手关节和杯子 6D 位姿。
- 将所有数据转换到统一世界坐标，单位统一为米。
- 增加原图 2D 投影和 3D 轨迹可视化。

完成标准：

- 手和杯子投影与视频画面基本重合。
- 世界坐标轨迹连续，没有镜像、跳变或尺度错误。

## 阶段 3：生成真实视频 MuJoCo demonstration

状态：已完成首条轨迹的 retarget、离屏回放和 demonstration 验证。

需要完成：

- 将转换后的手部数据送入 DexMV retarget。
- 对齐杯子轨迹、MuJoCo 桌面和 YCB mug 模型。
- 生成并检查 `relocate-mug-real.pkl`。
- 在 MuJoCo 中回放完整轨迹。

完成标准：

- Adroit 手能够接近并带动杯子。
- 不存在明显穿透、坐标翻转和逐帧跳动。

## 阶段 4：拆分可复用技能

需要完成：

- 定义技能标注格式。
- 自动检测并人工检查 `reach`、`grasp`、`lift`、`transport` 边界。
- 为每个技能导出独立 demonstration。
- 定义启动、成功、终止和超时条件。

完成标准：

- 每个片段都可以独立回放。
- 技能边界对应明确的物理事件。

## 阶段 5：训练低层技能策略

需要完成：

- 为每个技能训练行为克隆初始策略。
- 使用 DAPG 和随机初始状态微调。
- 收集 rollout 数据并训练技能可行性模型。
- 生成每个技能的评估报告和失败分类。

完成标准：

- 每个技能能在未见过的初始杯子位姿下稳定完成。
- 失败可以分类，而不是只记录总 reward。

## 阶段 6：实现技能执行框架

需要完成：

- 建立技能注册表和统一调用接口。
- 增加观测、超时、重试、恢复和日志功能。
- 通过反馈依次执行 `reach -> grasp -> lift -> transport`。

完成标准：

- 固定的符号计划可以完成杯子搬运。
- 某个技能失败时，系统会停止、重试或恢复。

## 阶段 7：增加高层语言规划器

需要完成：

- 先实现确定性的“指令到计划”规则基线。
- 定义严格的技能计划 JSON Schema。
- 收集指令、场景、计划和恢复样本。
- 数据足够后，对预训练语言模型做 LoRA/SFT。
- 用语言相关性和可行性分数共同选择技能。

完成标准：

- 同一任务的不同说法都能生成合法计划。
- 计划只调用当前场景中可执行的技能。
- 日志能显示每一步技能和成功概率。

## 阶段 8：增加倒水任务

需要完成：

- 录制 `tilt`、`upright`、`place` 和 `release` 真实视频。
- 增加目标容器和 `pour-mug` MuJoCo 环境。
- 实现基于杯口位置、倾斜角和保持时间的成功条件。
- 训练新增低层技能并扩展高层技能词表。

完成标准：

- 输入“拿起杯子并倒入目标容器”后，系统能规划和执行完整技能序列。

## 阶段 9：完整评估

需要完成：

- 测试未见过的受试者、杯子位姿、相机视角和指令表达。
- 分别统计感知、retarget、技能、规划和完整任务失败。
- 在真实机器人迁移前加入仿真域随机化。

完成标准：

- 可以从版本化配置和指定数据重新得到结果。
- 每次失败都能归因到感知、规划或控制层。

## 当前立即要做的工作

旧示范的 20 次 DAPG 短训练已完成，但旧策略没有在物理回放中抬杯。现在已从第一条 DexYCB 视频生成物理可执行示范，并在 11 个近邻初态通过验收；六条示范只是同一视频的扰动副本。下一步先做新示范的 20 次短训和独立策略评估，再扩展到多条原始视频。见[视频学习实施设计](VIDEO_IMITATION_PLAN.md)。
