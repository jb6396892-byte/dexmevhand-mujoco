...
B2 和 B5 已完成，详见 [MANO 与短训练记录](run_logs/2026-09-24-mano-smoke20.md)。接下来执行 C1：

1. 比较真实示范与训练环境重置时的手杯初始位置。
2. 筛选 Subject 01 的第二条右手 `025_mug` 轨迹并逐条验收。
3. 扩充到多条示范，按原始序列隔离训练和评估数据。
4. 统计接触和抬杯成功率，再决定是否延长训练。
