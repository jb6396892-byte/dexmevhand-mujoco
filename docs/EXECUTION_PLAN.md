...
- [x] B2：编写 DexYCB 数据扫描和转换程序，并完成 MANO 旋转对比。
- [x] B3：验证手、杯子和相机坐标。
- [x] B4：生成并回放第一条真实视频 demonstration。
- [x] B5：使用真实 demonstration 完成一次短训练。
- [ ] C0：用物理验收示范短训并独立评估学到的策略。
- [ ] C1：扩展到多条真实抓杯轨迹。
- [ ] C2：把完整轨迹拆成低层技能。
- [ ] C3：分别训练低层技能策略。
- [ ] C4：实现技能注册表和顺序执行器。
...
Subject 01 的真实序列上通过扫描、转换和重投影验收。MANO 姿态已通过标签验收；
几何框架的旋转差异已记录。
...
旧 B5 的 20 次短训仅证明示范格式、采样和策略加载可用；旧策略没有物理抬杯。首条真实序列现已生成六条毫米级扰动的可执行示范，并通过自由杯子动力学检查。六条示范属于**同一视频**，新策略尚未训练。

1. 在有共享盘数据和 dexmv 环境的机器上运行 `bash scripts/27_train_verified_smoke.sh`，检查示范、课程复位、采样器和 CUDA。
2. 运行 `bash scripts/27_train_verified_smoke.sh --train` 做 20 次 DAPG 短训，保存独立 checkpoint 和日志。
3. 用新 checkpoint 在独立复位种子上通过 `env.step(action)` 评估；记录接触、杯底高度、目标距离、穿透和动作饱和率。不得把 `scripts/28_view_verified_grasp_gpu.sh` 的专家动作回放当成学到的策略。
4. 分别判定“格式通过”“训练链路通过”“策略物理抓取成功”。未通过时留在 C0 诊断，不启动完整训练。

完整的视频学习设计见[实施设计](VIDEO_IMITATION_PLAN.md)，已验证专家示范的测量见[运行记录](run_logs/2026-09-26-physical-grasp-training-ready.md)。
...
B2 和旧 B5 已完成，但旧策略没有抓起杯子。当前先执行 C0：用物理验收示范做 20 次短训，并在独立复位种子上验证**学到的策略**。只有策略能稳定抬起自由杯子，才进入 C1 扩充独立真实视频；不要把同一序列的毫米级扰动副本算作多条视频。
